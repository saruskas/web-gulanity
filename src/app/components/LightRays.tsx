...
const getAnchorAndDir = (origin: LightRaysOrigin, w: number, h: number) => {
  const outside = 0.2;
  switch (origin) {
    case "top-left":
...
void main() {
  ...
  gl_FragColor = color;
      ...
      const updatePlacement = () => {
        ...
        rendererInstance.dpr = Math.min(window.devicePixelRatio, 2);
        const { clientWidth: wCSS, clientHeight: hCSS } = containerRef.current;
        rendererInstance.setSize(wCSS, hCSS);
        const dpr = rendererInstance.dpr;
        const w = wCSS * dpr;
        const h = hCSS * dpr;
